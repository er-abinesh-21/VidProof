...
serve(async (req) => {
  ...
  try {
    ...
    if (score === 100) {
      ...
      summary = "The video appears to be largely authentic. No direct signs of tampering were found, but some minor artifacts are present.";
    ...
    if (score < 100 && issues.length === 0) {
        issues.push({ timestamp: "00:00:10", description: "General integrity check passed with minor warnings.", severity: "low" });
    }
